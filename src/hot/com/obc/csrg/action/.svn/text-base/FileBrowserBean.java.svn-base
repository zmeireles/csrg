...
import com.obc.csrg.local.FileBrowserBeanLocal;
import com.obc.csrg.local.StateBeanLocal;

@Stateless
@Name("fileBrowser")
public class FileBrowserBean  implements FileBrowserBeanLocal {
	...
	String userFolder = "admin/";
	  ...
	  public String buildFileList(){
			...
			this.text.append("<ul class=\"fileThumbs\">");
	    ...
	    if (new File(dir).exists()) {
			...
			for (String file : files) {
			    ...
			    if (!new File(dir, file).isDirectory() && (!fileType.contains("image"))) {
					...
					html = "<li><a href=\"" + fileForView+ "\" title=\"" + file + "\">";
					html = html + "<div id=\"fileThumbs\" class=\"default " + ext + "\"></div>";
					html = html + file+ "</a></li>";
